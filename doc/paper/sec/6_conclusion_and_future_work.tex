...
In this work, we investigated a cut-and-paste outlier exposure strategy for improving out-of-distribution (OOD) detection in semantic segmentation models. The approach leverages real object instances from COCO and integrates them into road-scene images using structured augmentation priors, namely scale, perspective, and road-placement priors.
...
Experimental results across multiple benchmarks, including Lost\&Found, Fishyscapes, Road Anomaly, Road Anomaly 21, and Road Obstacle 21, show changes in OOD detection performance after fine-tuning. Improvements are most visible in false-positive suppression and in small-obstacle behavior on RoadObstacle21, while RoadAnomaly and RoadAnomaly21 reveal clear regressions. Performance therefore varies across datasets, indicating that the effectiveness of synthetic outlier exposure depends strongly on scene structure and anomaly characteristics.
...
Second, the object and placement sampling distributions are currently fixed by hand. More principled approaches could be explored, such as learning the sampling distribution jointly with the model or conditioning pasted object category, scale, and position on scene context.
...
Fifth, parameter-efficient adaptation methods such as LoRA~\cite{hu2022lora} could be explored to tune selected attention or prediction-head weights under limited compute budgets.
